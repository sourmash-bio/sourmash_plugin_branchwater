...
def test_simple_sigcat(runtmp, capfd):
    ...
    runtmp.sourmash(
        "scripts", "sigcat", query_list, against_list, "-o", output, "--verbose"
    )
...
def test_simple_sigcat_with_zip(runtmp, capfd):
    ...
    runtmp.sourmash("scripts", "sigcat", query_list, against_list, "-o", output)
    assert os.path.exists(output)
    assert not runtmp.last_result.out  # stdout should be empty
...
def test_sigcat_incompatible_sigfiles(runtmp, capfd):
    ...
    with pytest.raises(utils.SourmashCommandFailed):
        runtmp.sourmash("scripts", "sigcat", sig47, sig63, "-o", output, "-k", "9")
    ...
    assert "No signatures could be written to the output file" in captured.err


def test_sigcat_compatible_and_incompatible_sigfiles(runtmp, capfd):
    # test compat + incompatible sig files
    ...
    # with pytest.raises(utils.SourmashCommandFailed):
    runtmp.sourmash(
        "scripts", "sigcat", sig47, sig63, sighp, "-o", output, "-m", "hp"
    )  # , '--force')
...
def test_sigcat_with_bad_input_type_fail(runtmp, capfd):
    ...
    captured = capfd.readouterr()
    print(captured.err)
    assert f"WARNING: could not load sketches from path '{fa1}'" in captured.err
    assert "Error: No signatures could be written to the output file." in captured.err
    assert not os.path.exists(output)


def test_sigcat_with_bad_input_type(runtmp, capfd):
    # test using bad input (fasta file instead of sig)
    sig47 = get_test_data("47.fa.sig.gz")
    fa1 = get_test_data("short.fa")
    output = runtmp.output("out.zip")
...
def test_sigcat_multk_multsc_multmol(runtmp, capfd):
    # test cat with multiple types of sigs -- no selection
...
def test_sigcat_multk_multsc_multmol_selectk(runtmp, capfd):
    ...
    runtmp.sourmash(
        "scripts", "sigcat", sig1, sig2, sig_hp, sig_sc100k, "-k", "21", "-o", output
    )
    assert os.path.exists(output)
    assert not runtmp.last_result.out  # stdout should be empty
...
def test_sigcat_multk_multsc_multmol_selectmoltype(runtmp, capfd):
    ...
    runtmp.sourmash(
        "scripts", "sigcat", sig1, sig2, sig_hp, sig_sc100k, "-m", "hp", "-o", output
    )
    assert os.path.exists(output)
    assert not runtmp.last_result.out  # stdout should be empty
