...
def test_fastmultigather_rocksdb(runtmp):
    ...
    status = db.fastmultigather_against(query_coll, db.selection(), 0, csv_out)
...
def test_fastmultigather_general(runtmp):
    ...
    csv_out = runtmp.output("xxx.csv")
    status = against_coll.fastmultigather_against(query_coll, 0, 100_000, csv_out)
    print(f"status: {status}")
    df = pandas.read_csv(csv_out)
    assert len(df) == 3
    keys = set(df.keys())
...
def test_fastmultigather_general_nomatch(runtmp):
    ...
    csv_out = runtmp.output("xxx.csv")
    res = against_coll.fastmultigather_against(query_coll, 0, 100_000, csv_out)
    assert 0, res
