...
def do_fastmultigather(
    ...
    if is_rocksdb:
        res = against_db.fastmultigather_against(
            query_coll, against_db.selection(), threshold_hashes, output  # @CTB
        )
    else:
        res = against_coll.fastmultigather_against(
            query_coll, threshold_hashes, scaled, output, save_matches=save_matches
        )
    ...
    if n_skipped:
        print(
            f"WARNING: skipped {n_skipped} query paths - no compatible signatures.",
            file=sys.stderr,
        )

    if n_failed:
        print(
            f"WARNING: {n_failed} query paths failed to load. See error messages above.",
            file=sys.stderr,
        )
